...
public class CAMF_CUCI_IMP_WT_RBFK extends ContextRecommender{

    protected Table<Integer, Integer, Double> icBias;
    protected Table<Integer, Integer, Double> ucBias;
    
    public CAMF_CUCI_IMP_WT_RBFK(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
        super(trainMatrix, testMatrix, fold);
        this.algoName = "CAMF_CUCI_IMP_WT_RBFK";
    }
            ...
            for (MatrixEntry me : trainMatrix) {
                ...
                for(int cond:getConditions(ctx)){
                    ...
                    for (int f = 0; f < numFactors; f++) {
                        ...
                        double sgdh = 4*gamma*euj*Math.exp(-gamma*euj*euj)*(P.get(u, f)+Q.get(j, f)) - regC*hcf;
                ...
                for (int f = 0; f < numFactors; f++) {
                    ...
                    double delta_u = wu > 0 ? 4*gamma*euj*Math.exp(-gamma*euj*euj) * (qjf+h_sum) - regU * puf/wu : 4*gamma*euj*Math.exp(-gamma*euj*euj) * (qjf+h_sum) - regU * puf;
                    double delta_j = wi > 0 ? 4*gamma*euj*Math.exp(-gamma*euj*euj) * (puf+h_sum) - regI * qjf/wi : 4*gamma*euj*Math.exp(-gamma*euj*euj) * (puf+h_sum) - regI * qjf;
