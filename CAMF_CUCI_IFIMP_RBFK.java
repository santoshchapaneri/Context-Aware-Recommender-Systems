...
public class CAMF_CUCI_IFIMP_RBFK extends ContextRecommender{
    protected DenseMatrix Y;
    protected Table<Integer, Integer, Double> icBias;
    protected Table<Integer, Integer, Double> ucBias;

    public CAMF_CUCI_IFIMP_RBFK(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
        super(trainMatrix, testMatrix, fold);
        this.algoName = "CAMF_CUCI_IFIMP_RBFK";
    }
            ...
            for (MatrixEntry me : trainMatrix) {
                ...
                for(int cond:getConditions(ctx)){
                    ...
                    for (int f = 0; f < numFactors; f++) {
                        ...
                        double sgdh = 4*gamma*euj*Math.exp(-gamma*euj*euj)*(P.get(u, f)+Q.get(j, f)+sum_ys[f]) - regC*hcf;
                ...
                for (int f = 0; f < numFactors; f++) {
                    ...
                    double delta_u = 4*gamma*euj*Math.exp(-gamma*euj*euj) * (qjf+h_sum) - regU * puf;
                    double delta_j = 4*gamma*euj*Math.exp(-gamma*euj*euj) * (puf+sum_ys[f]+h_sum) - regI * qjf;
                    ...
                    for (int k : items) {
                        ...
                        double delta_y = 4*gamma*euj*Math.exp(-gamma*euj*euj) * (qjf + h_sum )/ w - regU * ykf;
