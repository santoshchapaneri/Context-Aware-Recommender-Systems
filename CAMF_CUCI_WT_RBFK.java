...
public class CAMF_CUCI_WT_RBFK extends ContextRecommender{

    protected Table<Integer, Integer, Double> icBias;
    protected Table<Integer, Integer, Double> ucBias;

    public CAMF_CUCI_WT_RBFK(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
        super(trainMatrix, testMatrix, fold);
        this.algoName = "CAMF_CUCI_WT_RBFK";
    }
    ...
    protected void buildModel() throws Exception {
        ...
        for (int iter = 1; iter <= numIters; iter++) {
            ...
            for (MatrixEntry me : trainMatrix) {
                ...
                loss1 += 2 * (1 - Math.exp(-gamma*euj*euj));
                ...
                for (int f = 0; f < numFactors; f++) {
                    ...
                    double delta_u = wu > 0 ? 4*gamma*euj*Math.exp(-gamma*euj*euj) * qjf - regU * puf/wu : 4*gamma*euj*Math.exp(-gamma*euj*euj) * qjf - regU * puf;
                    double delta_j = wi > 0 ? 4*gamma*euj*Math.exp(-gamma*euj*euj) * puf - regI * qjf/wi : 4*gamma*euj*Math.exp(-gamma*euj*euj) * puf - regI * qjf;
